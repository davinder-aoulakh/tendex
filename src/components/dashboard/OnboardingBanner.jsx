    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(0,201,167,0.12) 0%, rgba(0,201,167,0.08) 100%)', border: '1px solid rgba(0,201,167,0.2)' }}
        >
          {/* Glow */}
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: 'radial-gradient(ellipse 60% 80% at 50% 0%, rgba(0,201,167,0.15) 0%, transparent 70%)' }} />
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-4">
              <Sparkles className="w-5 h-5" style={{ color: '#00C9A7' }} />
              <h3 className="font-semibold text-white">Welcome to TendeX! Here's how it works:</h3>
            </div>
            <div className="grid sm:grid-cols-4 gap-4 mb-5">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: 'rgba(0,201,167,0.15)', border: '1px solid rgba(0,201,167,0.3)' }}>
                    <step.icon className="w-3.5 h-3.5" style={{ color: '#00C9A7' }} />
                  </div>
                  <div>
                    <div className="text-xs font-semibold text-white mb-0.5">{step.title}</div>
                    <div className="text-xs" style={{ color: 'rgba(0,201,167,0.5)' }}>{step.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-3">
              <Link to="/tool-select">
                <Button size="sm" className="gap-1.5 text-white border-0" style={{ backgroundColor: '#00C9A7', boxShadow: '0 0 20px rgba(0,201,167,0.3)' }}>
                  Create your first document <ArrowRight className="w-3.5 h-3.5" />
                </Button>
              </Link>
              <button onClick={dismiss} className="text-xs transition-colors" style={{ color: 'rgba(0,201,167,0.4)', marginLeft: '0.75rem' }} onMouseEnter={(e) => e.target.style.color = 'white'} onMouseLeave={(e) => e.target.style.color = 'rgba(0,201,167,0.4)'}>Dismiss</button>
            </div>
          </div>
        </motion.div>
      )}
